...
def printTable(table):
    ...
    :param table: 대상 테이블
    """
    rowSize = len(table.rows)
    colSize = len(table.rows[0].cells)
    log.debug("\nprint_table() rows size:"+ str(rowSize)+", column size:"+ str(colSize))

    for row in range(rowSize):
        rowStr = ""
        for col in range(colSize):
            if(len(table.rows[row].cells[col].text) > 0):
                rowStr = rowStr+table.rows[row].cells[col].text+"\t|\t"
            #log.debug(table.rows[row].cells[col].text+"\t")
        if(len(rowStr) > 0):
            log.debug('['+str(row)+'] '+rowStr.replace('\n', ''))
...
def loadWeeklyTop(dst, dayOfWeek):
    """ 
    ...
    :return: 없음
    """
    dst.rows[1].cells[1].text = getWeeklyDate(dayOfWeek)
...
def getColRowSize(table):
    ...
    """
    rowSize = len(table.rows)
    rowSize = rowSize - getBlankRowCount(table)
